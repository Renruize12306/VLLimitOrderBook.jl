...
function assing_valiables(scale, dir)
    num_sent_vec_file = open("$(dir)num_sent_vec.txt")
    time_sent_vec_file = open("$(dir)time_sent_vec.txt")
    time_rec_vec_file = open("$(dir)time_rec_vec.txt")
...
dir = "test/figures_input/server_client/data/"
scale = 100
result_vec = assing_valiables(scale, dir)
...
savefig("$(dir)latency_$(scale).pdf")
...
x_array = result_vec[1]
# x_array = x_array[1 : end - 1]
y_array = result_vec[2] - result_vec[3]
# y_array_sent = y_array_sent[1 : end - 1]

# y_array_rec = y_array_rec[1 : end - 1]

scatter(x_array, y_array, label="Latency from sending side", mc=:white, msc=colorant"#EF4035", legend=:best, 
bg="floralwhite", background_color_outside="white", framestyle=:box, fg_legend=:transparent, lw=3)
xlabel!("Number of Orders Sent", fontsize=18)
ylabel!("Latency (nanoseconds)", fontsize=18)

savefig("$(dir)latency_diff_$(scale).pdf")
...
# include("test/figures_input/server_client/data/talking_test_fig.jl")